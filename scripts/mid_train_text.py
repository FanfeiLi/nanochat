"""
Mid-train (continued pretraining) a base model on a mixed corpus.

Fork of base_train.py with three targeted changes for the mid-training phase:
  1. Warm-start model weights from an EXTERNAL checkpoint directory
     (--init-from-ckpt-dir / --init-from-step). The optimizer state is NOT
     loaded; we start with a fresh optimizer so stale momentum from base
     pretraining doesn't fight the new data distribution.
  2. Start step counter at 0 and use a FRESH LR schedule (warmup + warmdown
     over --num-iterations). This is a new training phase.
  3. Scale all learning rates by --lr-scale (default 0.25).

Output dir is controlled by NANOCHAT_BASE_DIR; saves land in
$NANOCHAT_BASE_DIR/base_checkpoints/d{depth}/ (same convention as base_train,
so downstream SFT can load it with source="base"). Use a DISTINCT base dir
from the original pretrain to avoid overwriting.

Typical usage:
torchrun --standalone --nproc_per_node=8 -m scripts.mid_train_text -- \\
    --depth=34 --device-batch-size=4 --fp8 --window-pattern=L \\
    --init-from-ckpt-dir=/fast/fli/.cache/nanochat_d34_newtok/base_checkpoints/d34 \\
    --init-from-step=38207 \\
    --num-iterations=500 \\
    --lr-scale=0.25 \\
    --warmup-steps=40 --warmdown-ratio=0.7 --final-lr-frac=0.0 \\
    --eval-every=100 --core-metric-every=-1 --save-every=250 \\
    --run=mid_arith_k5
"""
...
# Mid-train warm start (loads model weights from an external checkpoint dir)
parser.add_argument("--init-from-ckpt-dir", type=str, default="", help="initialize model weights from this checkpoint dir (external to NANOCHAT_BASE_DIR). Does NOT load optimizer state.")
parser.add_argument("--init-from-step", type=int, default=-1, help="step number of the external checkpoint to load (requires --init-from-ckpt-dir)")
parser.add_argument("--lr-scale", type=float, default=0.25, help="multiplier applied to all LRs after the base-train muP scaling. Default 0.25 = ¼ of base peak.")
# Evaluation
...
warm_start = bool(args.init_from_ckpt_dir) and args.init_from_step != -1
if resuming and warm_start:
    raise ValueError("--resume-from-step and --init-from-ckpt-dir are mutually exclusive")
if resuming:
    ...
    # Mid-train warm start: load model weights only, from an EXTERNAL dir.
    # No optimizer state, no meta, no dataloader resume — this is a fresh
    # training phase that just inherits the weights of the base model.
    print0(f"Warm-start from {args.init_from_ckpt_dir} step {args.init_from_step} (weights only)")
    model_data, _, _ = load_checkpoint(args.init_from_ckpt_dir, args.init_from_step, device, load_optimizer=False, rank=ddp_rank)
    model.load_state_dict(model_data, strict=True, assign=True)
    del model_data
...
# For mid-training, apply --lr-scale on top of the muP batch scaling (so the
# pretrain-trained model doesn't get its weights jolted too hard).
if warm_start and args.lr_scale != 1.0:
    print0(f"Mid-train LR scale: {args.lr_scale} (applied on top of muP batch_lr_scale={batch_lr_scale:.4f})")
_mid_scale = args.lr_scale if warm_start else 1.0
optimizer = model.setup_optimizer(
    # AdamW hyperparameters
    unembedding_lr=args.unembedding_lr * batch_lr_scale * _mid_scale,
    embedding_lr=args.embedding_lr * batch_lr_scale * _mid_scale,
    scalar_lr=args.scalar_lr * batch_lr_scale * _mid_scale,
    # Muon hyperparameters
    matrix_lr=args.matrix_lr * batch_lr_scale * _mid_scale,
...
if resuming:
    ...
    del optimizer_data
# NOTE: for warm_start we intentionally do NOT load optimizer state — fresh momentum.
